...
3. **Goose Desktop** hosts a private activation broker. A `run` request always creates a new Desktop window, backend lease, and session with the requested cwd and an auto-submitted initial message. Desktop returns `accepted` only after the renderer creates the session, applies the requested model selection, stores the first message, and starts submission.
...
One MSIX installs and versions the whole Windows product. Goose Desktop remains at the package root with its existing `resources\bin\goose.exe` CLI, while Launcher and its native dependencies live under `launcher`. Launcher derives both executable paths from this package layout and does not search PATH, registry protocol handlers, or unrelated Goose installations. The optional `GOOSE_WINDOWS_ROOT` override exists only for local development. Standard and CUDA packages use different fixed identities and must not be installed side by side.
